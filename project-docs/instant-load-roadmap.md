...
- 2026-07-13: Separate session picked up a real, unrelated bug report on this same branch: "Watch shows no plans to start a workout from." Root cause was the same class of issue as the earlier `navigator.onLine`/dead-cookie fixes further back in this branch's history — `NativeWatchWorkoutSync` (and, it turned out, `NativeHealthSync`, `NativePushRegister`, `ActiveWorkoutBanner`, `PreviousLogsCacheWarmer` too) gated their effects on `useSession().status === "authenticated"`, and that cookie-backed session can silently die on native while the real Bearer-token login stays valid — so the Watch's plan catalog never got pushed at all for an affected user, with everything else in the app working normally. Fixed by gating on the actual token (`getCachedToken()`) instead. This local fix (`a9c05c4`) landed shortly after another parallel session had already pushed 4 commits to `origin/feature/instant-load-cache-first` (`dd8d3a7`…`3e3e881`) that included an independently-diagnosed fix for the exact same root cause in `previous-logs-cache-warmer.tsx` and `native-health-sync.tsx`, via a different (arguably simpler) approach: removing the `useSession()` gate entirely rather than replacing it with a token check, since the underlying `fetch` calls are already self-guarding (the global `NativeAuthFetchPatch` attaches the real token; an unauthenticated request just 401s and bails harmlessly). Merged both branches (`ad6114a`), resolving the two conflicting files by adopting the simpler no-gate approach for consistency, keeping the token-gate approach only for `active-workout-banner.tsx` (untouched by the parallel session, and that one specifically needed to distinguish "not checked yet" from "confirmed logged out" for its render guard, not just its fetch). Re-verified clean (`tsc`, `xcodebuild`, `sync:native`) and pushed.
