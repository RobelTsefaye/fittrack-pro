...
export function Sidebar({ open, onClose }: SidebarProps) {
  ...
  return (
    <aside
      className={cn(
        "fixed inset-y-0 left-0 z-50 flex w-[min(18rem,calc(100vw-2.5rem))] max-w-[85vw] flex-col border-r bg-sidebar shadow-xl transition-transform safe-x-pad safe-top-offset lg:static lg:inset-y-auto lg:max-w-none lg:translate-x-0 lg:shadow-none",
        open ? "translate-x-0" : "-translate-x-full"
      )}
    >
      <div className="flex h-14 items-center justify-between border-b px-4">
        <Link href={ROUTES.dashboard} prefetch className="text-lg font-bold tracking-tight">
          {APP_NAME}
        </Link>
        <Button
          variant="ghost"
